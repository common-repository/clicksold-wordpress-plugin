...
<div id="ws_page_settings" class="cs-module">
  <?php 
	...
	$cs_opt_use_set_404_for_listing_unavailable = get_option("cs_opt_use_set_404_for_listing_unavailable", 0);
	$cs_opt_cloudflare_script_compatability = get_option("cs_opt_cloudflare_script_compatability", 0);
	$cs_filter_org_req_parameters = get_option("cs_filter_org_req_parameters", "");
	$cs_opt_skip_gmap_include = get_option("cs_opt_skip_gmap_include", 0);
	$cs_opt_skip_gmap_include_bo = get_option("cs_opt_skip_gmap_include_bo", 0);
	...
	if( isset($_POST["post_success"]) ) {
		...
		if( $valid == true ) {
			...
			if( isset( $_POST["cs_opt_use_set_404_for_listing_unavailable"] ) ) {
				update_option("cs_opt_use_set_404_for_listing_unavailable", 1);
				$cs_opt_use_set_404_for_listing_unavailable = get_option("cs_opt_use_set_404_for_listing_unavailable");
			} else {
				update_option("cs_opt_use_set_404_for_listing_unavailable", 0);
				$cs_opt_use_set_404_for_listing_unavailable = get_option("cs_opt_use_set_404_for_listing_unavailable");
			}

			if( isset( $_POST["cs_opt_cloudflare_script_compatability"] ) ) {
				update_option("cs_opt_cloudflare_script_compatability", 1);
				$cs_opt_cloudflare_script_compatability = get_option("cs_opt_cloudflare_script_compatability");
			} else {
				update_option("cs_opt_cloudflare_script_compatability", 0);
				$cs_opt_cloudflare_script_compatability = get_option("cs_opt_cloudflare_script_compatability");
			}

			if( isset( $_POST["cs_opt_skip_gmap_include"] ) ) {
				update_option("cs_opt_skip_gmap_include", 1);
				$cs_opt_skip_gmap_include = get_option("cs_opt_skip_gmap_include");
			} else {
				update_option("cs_opt_skip_gmap_include", 0);
				$cs_opt_skip_gmap_include = get_option("cs_opt_skip_gmap_include");
			}

			if( isset( $_POST["cs_opt_skip_gmap_include_bo"] ) ) {
				update_option("cs_opt_skip_gmap_include_bo", 1);
				$cs_opt_skip_gmap_include_bo = get_option("cs_opt_skip_gmap_include_bo");
			} else {
				update_option("cs_opt_skip_gmap_include_bo", 0);
				$cs_opt_skip_gmap_include_bo = get_option("cs_opt_skip_gmap_include_bo");
			}
    ...
    <div class="cs-semiopacity">
      <div id="ws_page_settings_form_manage_menus" class="cs-form-section">
        <fieldset class="cs-page-settings">
          ...
          <div class="cs-input-small">
            <div class="cs-label-container">
              <label for="cs_opt_use_set_404_for_listing_unavailable">Use set_404 for listing details not available pages:<span class="cs-required-field">*</span>:</label>
            </div>
            <input type="checkbox" name="cs_opt_use_set_404_for_listing_unavailable" value="1" <?php if($cs_opt_use_set_404_for_listing_unavailable){ ?>checked="checked"<?php } ?> class="cs-checkbox"/>
          </div>
          <div class="cs-input-small">
            <div class="cs-label-container">
              <label for="cs_opt_cloudflare_script_compatability">CloudFlare service script compatability (disables RocketScript for CS components):<span class="cs-required-field">*</span>:</label>
            </div>
            <input type="checkbox" name="cs_opt_cloudflare_script_compatability" value="1" <?php if($cs_opt_cloudflare_script_compatability){ ?>checked="checked"<?php } ?> class="cs-checkbox"/>
          </div>
          <div class="cs-input-small">
            <div class="cs-label-container">
              <label for="cs_opt_skip_gmap_include">Skip GMap Includes (Site):<span class="cs-required-field">*</span>:</label>
            </div>
            <input type="checkbox" name="cs_opt_skip_gmap_include" value="1" <?php if($cs_opt_skip_gmap_include){ ?>checked="checked"<?php } ?> class="cs-checkbox"/>
          </div>
          <div class="cs-input-small">
            <div class="cs-label-container">
              <label for="cs_opt_skip_gmap_include_bo">Skip GMap Includes (Admin Area):<span class="cs-required-field">*</span>:</label>
            </div>
            <input type="checkbox" name="cs_opt_skip_gmap_include_bo" value="1" <?php if($cs_opt_skip_gmap_include_bo){ ?>checked="checked"<?php } ?> class="cs-checkbox"/>
          </div>
        </fieldset>
      </div>
    </div>
