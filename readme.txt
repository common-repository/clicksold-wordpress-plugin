...
= 1.90 =
* Fix compatability with WordPress 5.3
* Support PHP 7.2

= 1.89 =
* Fix compatability with WordPress 5.1

= 1.88 =
* Fix CS IDX QuickSearch Widget when going to map view.
* CS Widgets no longer use old style constructors preventing warnings if wp debug is set to on.

= 1.87 =
* Split up the GMaps skip include call so that we can target the site and / or the admin area.

= 1.86 =
* Fix compatability with WordPress 4.7

= 1.85 =
* Fix compatability with WordPress 4.6

= 1.84 =
* Updated tested up to wp 4.5
* Upstream - Added Victoria region (VREB)

= 1.83 =
* Add option to skip gmaps include.

= 1.82 =
* Fix compatability with PHP < 5.3 - removed anonymous function calls.

= 1.81 =
* Fix issue with getting user capabilities when mobile version of the site is disabled.

= 1.80 =
* Fix compatability with Gmail SMTP plugin (and likely others, incompatability was due to including pluggable.php)
* Fix IDX Quick Search and Community Search Widgets when used on subdirectory wordpress installs.
* Upstream - TREB - Sold / Expired etc... listings are now automatically marked as such.

= 1.79 =
* IDX Quick Search Widget - now has Community searching built in and an option to either forward to the communties section or the mls search map.
* Upstream - Calgary - Added Property Sub Type field and mls search filter.
* Upstream - Calgary, Vancouver, RedDeer, Chicago, Austin, My Florida Regional and Grande Prairie - Added Open House search filter.
* Upstream - Added Extra MLS Search Filter capability to the Communities Section.
* Upstream - Community Search Widget and IDX Listings Quick Search Widget both now have the "All Communties" options where applicable.
* Upstream - Added Open House data for Edmonton (and it's associated open house search filter).
* Upstream - Add Edmonton Building Type search filter.

= 1.78 =
* Add shortcode - cs_contact_page - shows the agents from the My Team section and a contact form.
* Add shortcode - cs_home_eval_form - shows home evaluation form.
* Add shortcode - cs_mortgage_calc - shows mortgage calculator form.
* Add shortcode - cs_mortgage_calc_link - wraps shortcode content(text/image) in a link that opens the Mortgage calculator in it's standard popup window.

= 1.77 =
* Fix page editor not working.

= 1.76 =
* Add cloudflare compatability option.

= 1.75 =
* Fix compatability with WordPress 4.4

= 1.74 =
* Add config option to featured listings widget (slick slider version) to configure responsive breakpoints.
* CS now initializes correctly when installed by clicksold systems (no longer needed to log into admin in order for initialization to proceed).
* Upstream - ERA added new rural search type to MLS(r) search.
* Upstream - RCIDX added new rental search for featured listings section.
* Upstream - ACTRIS and RCIDX now include RealTrack tracking code as per board requirements.
* Upstream - AddThis widget now restricts options that won't work w/o a publication id.
* Upstream - Misc bugfixes.

= 1.73 =
* Extra styling clues for IDX Quick Search Widget (wide vs. narrow view).
* Better theme compatability to set wide pages for the mls search page automatically.
* fix warning during shortcode processing for pages that don't have post_content defined.
* Featured Listings Widget (slick slider based) now has optional prev / next arrows.
* Fix IDX Quick Search widget and Listings Quick Search widget on subdirectory installs.
* Upstream - Mobile site mls search can now be configured to open in options / list view automatically.
* Upstream - Added new RCIDX region (Michigan)

= 1.72 =
* Live chat help now appears in CS admin area.
* Featured Listings Widget - Added configuration options and new slick slider based slider.
* IDX Quick Search Widget - fixes, added wide view and force controls type option.
* Compatability with non genesis themes.
* Upstream - Allow hosted account to set the redirect to primary option.
* Upstream - My Domain section fixes.
* Upstream - Silver and Gold packages now get free month's trial (Platinum already had this).
* Upstream - Update CMHC mortgage insurance rates for Canadian accounts.
* Upstream - Add option so Mobile site MLS search can open directly on list view.

= 1.71 =
* Unavailable listing details pages now correctly return a 404 status code to please google.

= 1.70 =
* Fix WP_Widget warnings due to api change in WP 4.3
* Added new Listing Details Page Widget - which shows map and streetview on listing details pages if present in the sidebar.
* Upstream - New MlsNumber CSV search filter.
