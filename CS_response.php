	...
	*/
	function __construct($data, $plugin_section = ''){
		...
		if(!$this->error_state) {
			...
			if( isset( $data['response']['code'] ) ) { $this->response_status_code = $data['response']['code']; }
	...
	*/
	...
	private function cs_set_includes($block, $skip_linked = 0, $skip_inline = 0){
		...
		foreach($this->resource_includes[$block] as $include_item) {
			...
			// Save the names of all of the scripts that have been included.
			if($include_item["type"] == "JS" || $include_item["type"] == "JS_JAWR" || $include_item["type"] == "WP_ENQ_JS") {
				array_push( $cs_included_script_names[ $block ], $include_item["name"]);
			}
		}
		
		// If they have selected the special option to not include the CS provided google scripts then we do that here.
		if( get_option( 'cs_opt_skip_gmap_include', 0 ) && !is_admin()) { // Front of the site.
			$this->deregister_gmaps();
		}
		if( get_option( 'cs_opt_skip_gmap_include_bo', 0 ) && is_admin()) { // Site backend.
			$this->deregister_gmaps();
		}
	}
	...
	/**
	 * Used by the option that prevents ClickSold from adding it's own GMaps scripts (if we have duplicate gmaps includes).
	 */
	private function deregister_gmaps() {
		
		// This is the name that CS uses to register it's gmaps.
		wp_deregister_script('gmap');
		
		// CS also registers blank scripts with these names, so that other plugins can't register gmaps.
		// NOTE: This is the same list as in googleMapsAPI_include_register.jsp
...
/*****************************************************************************************/
	...
	*/
	public function get_body_contents( $org_content = "" ){
		...
		# Cloudflare changes the <script type="text/javascript" to text/rocketscript and this breaks the back office.
		if( get_option("cs_opt_cloudflare_script_compatability", 0) ) {
			$result = $result . str_replace('<script ', '<script data-cfasync="false" ', $this->response_contents["body_" . $this->current_response_section]);
		} else {
			$result = $result . $this->response_contents["body_" . $this->current_response_section];
		}
