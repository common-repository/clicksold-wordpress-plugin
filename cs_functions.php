...
function cs_save_cs_widgets_state() {
	...
	foreach( get_option('sidebars_widgets', array() ) as $sidebar_name => $sidebar_widget_list ) {
		...
		// If the value (sidebar_widget_list) is not an array we skip it as there are other keys in the sidebars_widgets array that are not actual arrays.
		if( !is_array( $sidebar_widget_list ) ) { continue; }
...
/**
 * Tells wordpress to send a 404.
 */
function cs_send_404() {
	http_response_code(404);
}

/**
 * Returns true if we are in one of the ClickSold admin section.
 */
function cs_is_cs_admin_section() {
	...
	// If it's one of our slugs, we're in the cs admin section.
	$found = 0;
	global $CS_ADMIN_MENU_ITEMS;
	
	// Escape for if cs_constants.php has not been loaded... this happens during plugin activation which is handled by CS_config.php where the full cs_listings_plugin.php is not actually yet loaded.
	// Note, we could just include cs_constants.php but for this routine it does not make sense, because we can't be in a cs admin section while the plugin is being activated.
	if( !isset($CS_ADMIN_MENU_ITEMS) ) { return; }
	
	foreach($CS_ADMIN_MENU_ITEMS as $key => $item){
		
		if( $item['menu_slug'] != '' && $item['menu_slug'] == $page_slug ) { // Remember that the Help menu item has a menu_slug of ''.
			$found = 1;
		}
	}
...
/**
 * Set the given page to full width, taking into account that cetain themes do this differently than others.
 */
function cs_set_full_width( $post_id ) {

	// Get a WP_Theme object with the details of the currently selected theme.
	$theme = wp_get_theme();
	
	if( $theme['Template'] == 'genesis' ) {											// Note we use "Template" here and not "Name" because all of the genesis ones have "Template" == genesis but have individual names.
		
		update_post_meta( $post_id, '_genesis_layout', 'full-width-content' );
		
	} else if(																		// ClickSold Themes
		$theme['Name'] == 'CS Anon' ||				// for cs themes we use the name directly as they don't have a common Template id.
		$theme['Name'] == 'Anon'
		//
		//
...
/**
 * Set the idx page to full width.
 */
function cs_set_idx_to_full_width() {

	global $wpdb;
	
	// Find the page id of the idx page.
	global $wpdb;
	global $cs_posts_table;

	// Get the one marked idx.
	$idx_post_id = $wpdb->get_results( "SELECT postid FROM " . $wpdb->prefix . $cs_posts_table . " where prefix = 'idx'" );

	// If we don't find anything return just bail.
	if( count( $idx_post_id ) == 0 ) {
		return;
	} else { // We did find the idx post id.
		
		// Set it to a wide screen page.
		cs_set_full_width( $idx_post_id[0]->postid );
	}
}
...
* @param string $handle
* 
* Stolen from http://snippets.webaware.com.au/snippets/stop-cloudflare-rocketscript-breaking-wordpress-plugin-scripts/
*/
function cs_no_rocketscript($handle) {
	global $wp_scripts;
...
function cs_no_rocketscript_handler( $block ) {
	global $cs_included_script_names;

	// For each script that cs has registered print it using the cs_no_rocketscript function.
	foreach( $cs_included_script_names[$block] as $script_name ) {

		// If enqueued, dequeue the script and print it using our function instead.
		if( wp_script_is( $script_name ) ) {
			wp_dequeue_script($script_name);
			cs_no_rocketscript($script_name);
		}
	}
}
